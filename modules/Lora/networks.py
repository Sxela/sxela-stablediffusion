...
def load_network(name, network_on_disk, sd_model):
    net = network.Network(name, network_on_disk)
    net.mtime = os.path.getmtime(network_on_disk.filename)

    sd = read_state_dict(network_on_disk.filename)

    # this should not be needed but is here as an emergency fix for an unknown error people are experiencing in 1.2.0
    if not hasattr(sd_model, 'network_layer_mapping'):
        assign_network_names_to_compvis_modules(sd_model)

    keys_failed_to_match = {}
    is_sd2 = 'model_transformer_resblocks' in sd_model.network_layer_mapping
    ...
    for key_network, weight in sd.items():
        ...
        sd_module = sd_model.network_layer_mapping.get(key, None)

        if sd_module is None:
            m = re_x_proj.match(key)
            if m:
                sd_module = sd_model.network_layer_mapping.get(m.group(1), None)
...
def load_networks(names, te_multipliers=None, unet_multipliers=None, dyn_dims=None, sd_model=None):
    already_loaded = {}
    ...
    for i, name in enumerate(names):
        ...
        if network_on_disk is not None:
            if net is None or os.path.getmtime(network_on_disk.filename) > net.mtime:
                try:
                    net = load_network(name, network_on_disk, sd_model)
                except Exception as e:
                    print(e, f"loading network {network_on_disk.filename}")
                    continue
        ...
        net.dyn_dim = dyn_dims[i] if dyn_dims else None
...
def network_restore_weights_from_backup(self: Union[torch.nn.Conv2d, torch.nn.Linear, torch.nn.MultiheadAttention]):
    ...
    #     self.in_proj_weight = weights_backup[0].clone()
    #     self.out_proj.weight= weights_backup[1].clone()
    # else:
    #     self.weight= weights_backup.clone()
...
def walk_files(path, allowed_extensions=None):
    ...
    for root, _, files in items:
        for filename in sorted(files, key=natural_sort_key):
            ...
            if not list_hidden_files and ("/." in root or "\\." in root):
                continue

            yield os.path.join(root, filename)


def list_available_networks(lora_dir='./'):
